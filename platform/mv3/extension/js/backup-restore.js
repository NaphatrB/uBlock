...
export async function backupToObject(currentConfig) {
    ...
    if ( tlEnabled || tlUrl || tlModel || tlApiKey || tlThinking ) {
        out.theyLive = {
            aiEnabled: Boolean(tlEnabled),
            aiBaseUrl: tlUrl || 'https://ollama.com',
            aiModel: tlModel || 'gemma4:31b-cloud',
            aiApiKey: tlApiKey || '',
            aiThinking: Boolean(tlThinking),
        };
    }
    return out;
}
...
export async function restoreFromObject(targetConfig) {
    ...
    if ( targetConfig.theyLive ) {
        ...
        await sendMessage({
            ...
            aiEnabled: Boolean(tl.aiEnabled),
            aiBaseUrl: tl.aiBaseUrl || 'https://ollama.com',
            aiModel: tl.aiModel || 'gemma4:31b-cloud',
            aiApiKey: tl.aiApiKey || '',
            aiThinking: Boolean(tl.aiThinking),
        });
    }
}
